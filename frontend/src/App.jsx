...
export default function App() {
  ...
  const formatNamasteResults = (results) => {
    ...
    return results.map((result, index) => (
      <div key={index} className="p-4 bg-white rounded-lg border border-amber-200 shadow-xs mb-3">
        <div className="flex justify-between items-start mb-2">
          <h4 className="font-semibold text-orange-700">Result {index + 1}</h4>
        ...
        <div className="text-sm space-y-1">
          ...
          {result.matched_columns && result.matched_columns.length > 0 && (
            <div className="flex mt-2 pt-2 border-t border-amber-100">
  ...
  return (
    <div className="min-h-screen w-full bg-amber-50 text-gray-900 flex flex-col relative overflow-hidden">
      {/* Decorative elements */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-orange-100 rounded-full mix-blend-multiply filter blur-xl opacity-30 animate-blob"></div>
      <div className="absolute top-1/4 left-10 w-80 h-80 bg-yellow-100 rounded-full mix-blend-multiply filter blur-xl opacity-30 animate-blob animation-delay-2000"></div>
      <div className="absolute bottom-0 right-1/4 w-64 h-64 bg-amber-100 rounded-full mix-blend-multiply filter blur-xl opacity-30 animate-blob animation-delay-4000"></div>
      
      <header className="bg-gradient-to-r from-orange-600 to-amber-600 text-white border-b-4 border-amber-800 relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-amber-100 text-sm">National Medical Coding Portal</p>
            </div>
          </div>

          <nav className="mt-2 sm:mt-0">
            <ul className="flex space-x-6 text-sm font-medium">
              <li className="hover:text-amber-200 cursor-pointer transition-colors duration-200 border-b-2 border-transparent hover:border-amber-200 pb-1">Home</li>
              <li className="hover:text-amber-200 cursor-pointer transition-colors duration-200 border-b-2 border-transparent hover:border-amber-200 pb-1">About</li>
              <li className="hover:text-amber-200 cursor-pointer transition-colors duration-200 border-b-2 border-transparent hover:border-amber-200 pb-1">Help</li>
            </ul>
          </nav>
        </div>
      </header>
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 grid grid-cols-1 lg:grid-cols-4 gap-8 relative z-10">
        {/* Left: Main App (3/4) */}
        <main className="lg:col-span-3 bg-white border border-amber-200 rounded-xl shadow-md p-6 relative overflow-hidden">
          {/* Decorative corner elements */}
          <div className="absolute top-0 right-0 w-24 h-24 overflow-hidden">
            <div className="absolute transform rotate-45 bg-orange-100 text-white shadow-md w-40 h-10 -right-12 top-5"></div>
          </div>
          
          <div className="absolute bottom-0 left-0 w-24 h-24 overflow-hidden">
            <div className="absolute transform rotate-45 bg-amber-100 text-white shadow-md w-40 h-10 -left-12 -bottom-5"></div>
          </div>
          
          <h2 className="text-xl font-semibold mb-6 text-orange-800 border-b-2 border-amber-200 pb-3 flex items-center">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="flex-1 relative">
                    ...
                    <input
                      ...
                      className="pl-10 w-full border border-amber-300 rounded-lg px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all bg-amber-50"
                  ...
                  <button 
                    ...
                    disabled={searchLoading}
                    className="bg-gradient-to-r from-orange-600 to-amber-600 text-white px-6 py-3 rounded-lg text-base font-medium hover:from-orange-700 hover:to-amber-700 disabled:opacity-70 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg flex items-center justify-center"
                  >
                    {searchLoading ? (
          ...
          {searchResults && activeTab === "search" && (
            <section className="space-y-6">
              ...
              {((!searchResults.namaste_matches || searchResults.namaste_matches.length === 0) &&
               (!searchResults.icd11_matches || searchResults.icd11_matches.length === 0)) && (
                <div className="border border-amber-200 rounded-lg p-5 bg-amber-50 shadow-sm">
                  <div className="flex items-center">
                    ...
                    <span className="text-amber-800 font-medium">No results found for "{searchResults.query}"</span>
                  </div>
                  <p className="text-amber-700 text-sm mt-2">Try different search terms or check the spelling.</p>
                </div>
              )}
            </section>
          )}
          ...
          {mappingResults && activeTab === "map" && (
            <section className="space-y-6">
              ...
              {mappingResults.namaste_info && Object.keys(mappingResults.namaste_info).length > 0 && (
                <div className="border border-orange-200 rounded-lg p-5 bg-orange-50 shadow-sm">
                  <h3 className="font-semibold text-orange-800 mb-3 text-lg">
              ...
              {(!mappingResults.icd11_matches || mappingResults.icd11_matches.length === 0) && (
                <div className="border border-amber-200 rounded-lg p-5 bg-amber-50 shadow-sm">
                  <div className="flex items-center">
                    ...
                    <span className="text-amber-800 font-medium">No ICD-11 matches found for "{mappingResults.namaste_code}"</span>
                  </div>
                  <p className="text-amber-700 text-sm mt-2">Try a different NAMASTE code or check the format.</p>
                </div>
              )}
            </section>
          )}
          ...
          {!searchResults && !mappingResults && !searchLoading && !mappingLoading && (
            <section className="border border-amber-200 rounded-lg p-5 bg-amber-50 shadow-sm">
              ...
              <div className="text-sm text-amber-800 space-y-2">
                {activeTab === "search" ? (
                  <>
                    <div className="p-3 bg-white rounded-lg border border-amber-100 shadow-xs">
        ...
        <aside className="lg:col-span-1 relative">
          <div className="sticky top-6 flex flex-col space-y-6">
            ...
            <div className="bg-white border border-amber-200 rounded-xl shadow-md p-5">
              <h3 className="text-lg font-semibold text-orange-800 mb-3 border-b border-amber-200 pb-2 flex items-center">
                <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
      ...
      {/* Footer */}
      <footer className="bg-gradient-to-r from-orange-700 to-amber-700 text-white border-t-4 border-amber-800 py-5 text-center mt-auto relative z-10">
        <div className="max-w-7xl mx-auto px-4">
          <p className="text-sm">© 2025 राष्ट्रीय स्वास्थ्य सूचना विज्ञान. सर्वाधिकार सुरक्षित.</p>
          <p className="text-amber-100 text-xs mt-1">© 2025 National Health Informatics. All Rights Reserved.</p>
        </div>
      </footer>
