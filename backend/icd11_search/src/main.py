#!/usr/bin/env python3
"""
ICD-11 Medical Code Search Engine - Main Application
==================================================

This is the main entry point for the ICD-11 search engine that uses
...
from icd11_search.src.data_loader import load_and_preprocess_data
from icd11_search.src.embedder import generate_and_save_embeddings, load_embeddings
from icd11_search.src.indexer import build_and_save_index, get_index_info
from icd11_search.src.search import ICD11Search, search_interactive, search_api
...
def print_banner():
    ...
    banner = """
    ╔══════════════════════════════════════════════════════════════╗
    ║              🏥 ICD-11 Medical Code Search Engine 🏥         ║
    ║                                                              ║
    ║        Semantic Search for ICD-11 Medical Codes             ║
...
def check_system_status():
    ...
    # Check data file
    data_path = os.path.join(base_dir, 'icd11_clean.csv')
    ...
    # Check FAISS index
    index_path = os.path.join(base_dir, 'index', 'icd11_faiss_index.bin')
...
def print_status():
    ...
    components = [
        ('Data File (icd11_clean.csv)', status['data_file']),
        ('Embeddings (icd11_embeddings.npy)', status['embeddings']),
        ('FAISS Index (icd11_faiss_index.bin)', status['index']),
    ]
...
def setup_pipeline(force_rebuild=False):
    ...
    """
    print("\n🚀 Setting up ICD-11 Search Engine Pipeline")
    print("=" * 55)
    ...
    try:
        ...
        if df is None:
            print("❌ Data loading failed. Please ensure icd11_clean.csv exists in the current directory.")
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ICD-11 Medical Code Search Engine",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main.py --setup                    # Set up the search engine
  python main.py --search                   # Interactive search mode
  python main.py --query "digestive power"  # Single query search
